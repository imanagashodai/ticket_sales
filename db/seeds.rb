...
ActiveRecord::Base.connection.execute("SELECT setval('blockedusers_id_seq', coalesce((SELECT MAX(id)+1 FROM blockedusers), 1), false)") 
...
ActiveRecord::Base.connection.execute("SELECT setval('games_id_seq', coalesce((SELECT MAX(id)+1 FROM games), 1), false)") 
ActiveRecord::Base.connection.execute("SELECT setval('tickets_id_seq', coalesce((SELECT MAX(id)+1 FROM tickets), 1), false)") 
ActiveRecord::Base.connection.execute("SELECT setval('carts_id_seq', coalesce((SELECT MAX(id)+1 FROM carts), 1), false)") 
